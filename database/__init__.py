...
def new_table(entity):
    if entity == 'Nodes':
        ...
        commit_db('CREATE TABLE Nodes('
                  ...
                  'Lat FLOAT,'
                  'Lon FLOAT)')
    elif entity == 'GraphShortest':
        commit_db('DROP TABLE IF EXISTS GraphShortest')
        commit_db('CREATE TABLE GraphShortest('
                  'FromID INTEGER,'
                  'ToID INTEGER,'
                  'Costs FLOAT);')
    elif entity == 'Bounds':
...
def get_shortest():
    ...
    return query_db('SELECT * FROM GraphShortest'), nodes
